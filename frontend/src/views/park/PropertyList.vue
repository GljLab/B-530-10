      ...
      <template #header>
        <div class="card-header">
          ...
          <div class="header-buttons">
            ...
            <el-button v-if="hasChangeStatusPermission && selectedIds.length > 0" type="warning" @click="handleBatchStatus">
              <el-icon><Edit /></el-icon>批量改状态
            </el-button>
            <el-button v-if="hasDeletePermission && selectedIds.length > 0" type="danger" @click="handleBatchDelete">
...
function handleBatchStatus() {
  ElMessageBox.prompt('请选择状态', '批量修改状态', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    inputPattern: /.+/,
    inputErrorMessage: '请选择状态'
  }).then(() => {
    ElMessage.info('请在弹窗中选择状态')
  }).catch(() => {})
}
